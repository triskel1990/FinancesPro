...
def tts_proxy():
    ...
    ELEVEN_KEY = os.environ.get('ELEVENLABS_API_KEY', 'sk_b5c343e89d0a1a7ce7d40b593e8959c343137adb4b959040')
...
def tts_debug():
    ...
    ELEVEN_KEY = os.environ.get('ELEVENLABS_API_KEY', 'sk_b5c343e89d0a1a7ce7d40b593e8959c343137adb4b959040')
    ELEVEN_VID = os.environ.get('ELEVENLABS_VOICE_ID', 'PSVUmed8NvS8aUA3d5oO')
    # Tester la cle sans generer audio
    try:
        resp = req.get(
        ...
        return jsonify({
            'key_prefix': ELEVEN_KEY[:12] + '...',
            'voice_id': ELEVEN_VID,
            'elevenlabs_status': resp.status_code,
            'elevenlabs_response': resp.json() if resp.status_code == 200 else resp.text[:200]
        })
    except Exception as e:
        return jsonify({'error': str(e), 'key_prefix': ELEVEN_KEY[:12] + '...'})
